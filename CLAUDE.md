...
- **The daemon is launched via `launch_daemon` (wrapper appends `[daemon exited rc=N]` to /tmp/awg_daemon.log)** — a silent signal death (132=SIGILL, 139=SIGSEGV) is distinguishable from a hang; interface-create failure retries once under `LOG_LEVEL=verbose` and continues if the retry succeeds. Keep new launch paths going through `launch_daemon`, or the rc line disappears.
