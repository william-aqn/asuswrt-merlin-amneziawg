...
- **amneziawg-go v0.2.19 requires Go 1.24.4 (go.mod), and Go ≥ 1.24 supports only Linux kernel ≥ 3.2** — on the RT-AC68U's 2.6.36 (all 386.x builds) the daemon is officially unsupported and dies unpredictably/silently (banner prints, then nothing — the rc line above is how it shows). A Go 1.23 rebuild needs a deep dependency downgrade (x/sys+x/net+x/crypto AND outline-sdk/x — each pinned version requires go ≥ 1.24 or a newer x/net) — attempted 2026-07, abandoned as too risky without on-device testing; revisit only with an AC68U in hand.
