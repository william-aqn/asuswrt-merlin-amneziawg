...
- **Firmware VPN client (wgc*/VPN Fusion) outranks AWG routing** — its policy rules sit at ip-rule prio <98 (VPN Fusion: `20: from all lookup 8437`), so a CONNECTED firmware VPN captures traffic before our fwmark rule; an enabled-but-idle profile (`wgc_enable=1`, dead endpoint — real field case) is the latent form. `fw_vpn_client_state()` probes both (unconditional from-all rules with a default route in their table = "active"; nvram wgc*_enable=1 = "enabled"; fwmark-gated rules excluded — that's xray_capture's territory; Merlin VPN Director's per-device `from <ip>` rules at prio >10000 don't outrank us and don't alarm) → status `fwvpn_state`/`fwvpn_detail` → red/yellow UI banner + journal + diag section.
