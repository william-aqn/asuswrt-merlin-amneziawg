...
- **Профили конфигурации** -- до 5 клиентских конфигов с именами и переключением в один клик (v1.4.0); опциональное **автопереключение при отказе**: не прошедший проверку связности туннель по кругу пробует следующий профиль, перезагрузка/ручной выбор возвращают основной; из SSH — `amneziawg.sh profile [list|N|next|slot:S]` (с 1.5.26 `N` — порядковый номер профиля в списке, `slot:S` — стабильный номер слота для скриптов)
