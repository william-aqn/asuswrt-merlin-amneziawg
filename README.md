...
- **Предупреждения** — баннеры при сером/CGNAT WAN-адресе (сервер не примет входящие — нужен белый IP или проброс порта), при конфликте порта со встроенным WG-сервером, и при активном Xray/XRAYUI в режиме «весь трафик» (перехватывает исходящий трафик и ломает двойной хоп; есть кнопка «Остановить Xray»).
