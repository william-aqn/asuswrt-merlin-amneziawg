...
- **Режим сервера (v1.3.0)** -- роутер сам принимает входящие AmneziaWG-подключения: доступ с телефона/ноутбука из любой точки к домашней сети и интернету через дом, с полной обфускацией (годится там, где чистый WireGuard режет DPI). Отдельная страница **VPN > AmneziaWG Server**: генерация ключей и **QR прямо в браузере**, управление пирами, per-peer политика маршрутизации (в т.ч. **двойной хоп** — трафик пира уходит в клиентский туннель). Клиент и сервер работают одновременно
...
- **Предупреждения о конфликтах** -- баннеры на странице: co-resident DPI/прокси (zapret/Xray/b4), XRAYUI в режиме «redirect all», VPN-клиент самой прошивки (wgc/VPN Fusion), чьи правила перекрывают правила аддона
