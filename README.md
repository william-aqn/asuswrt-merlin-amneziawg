...
> - **[PR #152](https://github.com/amnezia-vpn/amneziawg-go/pull/152)** — ограниченный буферный пул (`PreallocatedBuffersPerPool`) с настройкой через переменную окружения `WG_PREALLOCATED_BUFFERS_PER_POOL`: лекарство от `runtime: out of memory` под нагрузкой на роутерах с малой памятью. Дефолт сборки — 1024; с 1.3.13 на роутерах с ≥ 768 MiB RAM аддон при запуске снимает лимит (`0` = как в штатном Linux-профиле upstream — полная скорость);
